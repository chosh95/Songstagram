...
public interface PostsRepository extends JpaRepository<Posts, Long> {
    ...
    Page<Posts> getPostsCntByUsersSet(Set<Users> users, Pageable pageable); // user 목록에 있는 user가 작성한 게시글 목록 페이징 후 반환

    @Query(value = "SELECT count(p) FROM Posts p WHERE p.users in ?1")
    Long getPostsCntByUsersSet(Set<Users> users); // user 목록에 있는 user가 작성한 모든 게시글의 수 반환
